...
case
when PK1.ORG = '220' then substr(PK1.CUENTA,1,11) ||''||'****'||''|| substr(PK1.CUENTA,16,4)
when PK1.ORG = '230' then substr(PK1.CUENTA,1,11) ||''||'****'||''|| substr(PK1.CUENTA,16,4)
else '0'
end as CUENTA,
...
when PK1.DMLABEL6 = '0311' then 'San Juan Del Río'
...
when PK1.DMLABEL6 = '47' then 'Irapuato'
...
where trunc(BEGINDT) = trunc(to_date('01/06/2025','dd/mm/yyyy'))
and (U6CHGOFFSTAT = '2' or (U6CHGOFFSTAT = '1' and U6BILLCYCLE <= '28'))
and DMCURBAL > '0'
and U6ORG in ( '220','230')
order by 1) PK1

LEFT OUTER JOIN

(select DMACCT as CUENTA, U5CHGOFFSTAT as STATUS_FIN, U5BLOCKCODE2 as B2, DMPAYDT as ULT_PAGO
from RCVRY.DELQMST, RCVRY.UDA5
...
from RCVRY.PMTFIL
where trunc(PMEDATE) >= trunc(to_date('01/06/2025','dd/mm/yyyy')) and trunc(PMEDATE) <= trunc(to_date('30/06/2025','dd/mm/yyyy'))
